...
class Listener {
    ...
    handleMessage(message, resolve) {
        if (message.uid) {
            ...
            this.messageSubject.next({
                ...
                message: new BlockInfo_1.BlockInfo(message.meta.hash, message.meta.generationHash, message.meta.totalFee ? UInt64_1.UInt64.fromNumericString(message.meta.totalFee) : new UInt64_1.UInt64([0, 0]), message.meta.numTransactions, message.block.signature, PublicAccount_1.PublicAccount.createFromPublicKey(message.block.signerPublicKey, message.block.network), message.block.network, message.block.version, message.block.type, UInt64_1.UInt64.fromNumericString(message.block.height), UInt64_1.UInt64.fromNumericString(message.block.timestamp), UInt64_1.UInt64.fromNumericString(message.block.difficulty), message.block.feeMultiplier, message.block.previousBlockHash, message.block.blockTransactionsHash, message.block.blockReceiptsHash, message.block.stateHash, message.block.beneficiaryPublicKey),
            });
        }
    ...
    newBlock() {
        ...
        return this.messageSubject.asObservable().pipe(operators_1.share(), operators_1.filter((_) => _.channelName === ListenerChannelName.block), operators_1.filter((_) => _.message instanceof BlockInfo_1.BlockInfo), operators_1.map((_) => _.message));
    }
    /**
    ...
    confirmed(address, transactionHash) {
        ...
        return this.messageSubject.asObservable().pipe(operators_1.filter((_) => _.channelName === ListenerChannelName.confirmedAdded), operators_1.filter((_) => _.message instanceof Transaction_1.Transaction), operators_1.map((_) => _.message), operators_1.filter((_) => this.transactionFromAddress(_, address)), operators_1.filter((_) => {
    ...
    aggregateBondedAdded(address, transactionHash) {
        ...
        return this.messageSubject.asObservable().pipe(operators_1.filter((_) => _.channelName === ListenerChannelName.aggregateBondedAdded), operators_1.filter((_) => _.message instanceof AggregateTransaction_1.AggregateTransaction), operators_1.map((_) => _.message), operators_1.filter((_) => this.transactionFromAddress(_, address)), operators_1.filter((_) => {
            if (transactionHash === undefined) {
                return true;
            }
            else {
    ...
    status(address) {
        ...
        return this.messageSubject.asObservable().pipe(operators_1.filter((_) => _.channelName === ListenerChannelName.status), operators_1.filter((_) => _.message instanceof TransactionStatusError_1.TransactionStatusError), operators_1.map((_) => _.message), operators_1.filter((_) => address.equals(_.address)));
    }
    /**
    ...
    transactionHasSignerOrReceptor(transaction, address) {
        if (address instanceof NamespaceId_1.NamespaceId) {
            return transaction instanceof TransferTransaction_1.TransferTransaction && transaction.recipientAddress.equals(address);
        }
        return (transaction.signer.address.equals(address) ||
            (transaction instanceof TransferTransaction_1.TransferTransaction && transaction.recipientAddress.equals(address)));
    }
    /**
     * @internal
    ...
    accountAddedToMultiSig(transaction, address) {
        if (transaction instanceof MultisigAccountModificationTransaction_1.MultisigAccountModificationTransaction) {
            return (transaction.publicKeyAdditions.find((_) => _.address.equals(address)) !== undefined ||
                transaction.publicKeyDeletions.find((_) => _.address.equals(address)) !== undefined);
        }
        return false;
    }
}
