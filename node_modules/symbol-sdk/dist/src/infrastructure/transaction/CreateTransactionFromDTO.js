...
const CreateStandaloneTransactionFromDTO = (transactionDTO, transactionInfo) => {
    if (transactionDTO.type === TransactionType_1.TransactionType.TRANSFER) {
        ...
        return new MultisigAccountModificationTransaction_1.MultisigAccountModificationTransaction(transactionDTO.network, transactionDTO.version, Deadline_1.Deadline.createFromDTO(transactionDTO.deadline), UInt64_1.UInt64.fromNumericString(transactionDTO.maxFee || '0'), transactionDTO.minApprovalDelta, transactionDTO.minRemovalDelta, transactionDTO.publicKeyAdditions
            ? transactionDTO.publicKeyAdditions.map((addition) => PublicAccount_1.PublicAccount.createFromPublicKey(addition, transactionDTO.network))
            : [], transactionDTO.publicKeyDeletions
            ? transactionDTO.publicKeyDeletions.map((deletion) => PublicAccount_1.PublicAccount.createFromPublicKey(deletion, transactionDTO.network))
    ...
    else if (transactionDTO.type === TransactionType_1.TransactionType.ACCOUNT_LINK) {
    ...
    if (transactionDTO.transaction.type === TransactionType_1.TransactionType.AGGREGATE_COMPLETE ||
        ...
        const innerTransactions = transactionDTO.transaction.transactions.map((innerTransactionDTO) => {
            const aggregateTransactionInfo = innerTransactionDTO.meta
                ? new AggregateTransactionInfo_1.AggregateTransactionInfo(UInt64_1.UInt64.fromNumericString(innerTransactionDTO.meta.height), innerTransactionDTO.meta.index, innerTransactionDTO.meta.id, innerTransactionDTO.meta.aggregateHash, innerTransactionDTO.meta.aggregateId)
                : undefined;
        ...
        return new AggregateTransaction_1.AggregateTransaction(transactionDTO.transaction.network, transactionDTO.transaction.type, transactionDTO.transaction.version, Deadline_1.Deadline.createFromDTO(transactionDTO.transaction.deadline), UInt64_1.UInt64.fromNumericString(transactionDTO.transaction.maxFee || '0'), innerTransactions, transactionDTO.transaction.cosignatures
            ...
            : undefined, transactionDTO.meta
            ? new TransactionInfo_1.TransactionInfo(UInt64_1.UInt64.fromNumericString(transactionDTO.meta.height), transactionDTO.meta.index, transactionDTO.meta.id, transactionDTO.meta.hash, transactionDTO.meta.merkleComponentHash)
            : undefined);
    }
    else {
