...
class AggregateTransaction extends Transaction_1.Transaction {
    ...
    static createFromPayload(payload) {
        ...
        const consignatures = builder.getCosignatures().map((cosig) => {
            return new AggregateTransactionCosignature_1.AggregateTransactionCosignature(format_1.Convert.uint8ToHex(cosig.signature.signature), PublicAccount_1.PublicAccount.createFromPublicKey(format_1.Convert.uint8ToHex(cosig.signerPublicKey.key), networkType));
        });
    ...
    get size() {
        ...
        this.innerTransactions.forEach((transaction) => {
            ...
            const paddedTransactionByte = catbuffer_typescript_1.GeneratorUtils.concatTypedArrays(transactionByte, innerTransactionPadding);
    ...
    generateBytes() {
        ...
        const cosignatures = this.cosignatures.map((cosignature) => {
            ...
            return new catbuffer_typescript_1.CosignatureBuilder(new catbuffer_typescript_1.KeyDto(signerBytes), new catbuffer_typescript_1.SignatureDto(signatureBytes));
        });
