...
class TransactionService {
    ...
    resolveAliases(transationHashes) {
        return this.transactionRepository.getTransactions(transationHashes).pipe(operators_1.mergeMap((_) => _), operators_1.mergeMap((transaction) => this.resolveTransaction(transaction)), operators_1.toArray());
    }
    /**
    ...
    getTransactionOrRaiseError(listener, address, transactionHash, transactionObservable) {
        const errorObservable = listener.status(address).pipe(operators_1.filter((t) => t.hash.toUpperCase() === transactionHash.toUpperCase()));
    ...
    checkShouldResolve(transaction) {
        switch (transaction.type) {
            case TransactionType_1.TransactionType.ACCOUNT_LINK:
